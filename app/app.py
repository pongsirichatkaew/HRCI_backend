...
def login():
    try:
        ...
        try:
            ...
            cursor.execute(sql, (username, hashlib.sha512(
                password).hexdigest(), check_employeeid[0]['code']))
            data = cursor.fetchall()
            columns = [column[0] for column in cursor.description]
            ...
            if response['status'] == 'success':
                ...
                if 'result' in response:
                    ...
                    if 'token_type' in response and 'access_token' in response:
                        accessTokenOneID = response['token_type'] + \
                ...
                authenticationOneID = getAuthorizationAPI(
                    "https://one.th/api/account_and_biz_detail", accessTokenOneID)
...
def callServer(path, data):
    ...
    try:
        response = requests.request(
            "POST", url=url, json=payload, verify=False, stream=True, timeout=99)
...
def getAuthorizationAPI(path, data):
    ...
    try:
        response = requests.get(
            url, headers={'Authorization': payload}, verify=False, stream=True, timeout=99)
        return {'status': 'success', 'response': response}
    except requests.HTTPError:
        current_app.logger.info("HTTP error occurred.")
        return {'status': 'HTTPError', 'message': "HTTP error occurred."}
