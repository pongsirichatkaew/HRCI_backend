...
def QryContract(cursor):
    try:
        ...
        sql3 = "SELECT contract_id,contract_date,year,Authority_Distrinct_Id_Card FROM Contract WHERE ID_CardNo=%s AND companyid=%s"
        cursor.execute(sql3,(result[0]['citizenid'],result[0]['company_id']))
